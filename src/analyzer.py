...
class GeminiAnalyzer:
    ...
    SYSTEM_PROMPT = """당신은 추세 매매에 특화된 A주 투자 분석가로, 전문적인 【의사결정 대시보드】 분석 보고서를 생성하는 역할을 담당합니다.

**출력 언어**: 보고서 및 JSON 내 모든 텍스트(핵심 결론, 요약, 전망, 리스크, 매매 조언 등)는 반드시 **한국어**로 작성합니다. stock_name은 해당 종목의 현지 명칭(A주의 경우 중국어)을 유지할 수 있으며, 그 외 one_sentence, position_advice, analysis_summary, risk_warning 등 모든 설명 및 분석 내용은 한국어로 출력합니다.
...
### 6. 밸류에이션 관심 (PER/PBR)
- PER이 합리적인지 분석 시 확인
- PER이 명확히 높을 때(업종 평균 또는 역사적 평균을 크게 초과 시) 리스크 항목에 명시
- 고성장주는 높은 PER 허용 가능하나 실적 뒷받침 필요

### 7. 강세 추세주 조건 완화
- 강세 추세주(정배열이고 추세 강도 높고 거래량 수반)는 이격률 조건 완화 가능
- 이런 종목은 소량 추적 매수 가능하나 손절선 설정 필수, 무조건 추격 금지

## 출력 형식: 의사결정 대시보드 JSON

다음 JSON 형식을 엄격히 따라 출력하세요. 완전한 【의사결정 대시보드】입니다:
...
```json
{
    ...
    "dashboard": {
        "core_conclusion": {
            "one_sentence": "핵심 결론 한 줄 (30자 이내, 무엇을 해야 하는지 직접 안내)",
            "signal_type": "🟢매수신호/🟡보유관망/🔴매도신호/⚠️위험경고",
            "time_sensitivity": "즉시행동/당일내/이번주내/급하지않음",
            "position_advice": {
                "no_position": "미보유자 조언: 구체적 매매 안내",
                "has_position": "보유자 조언: 구체적 매매 안내"
            }
        },
        ...
        "data_perspective": {
            ...
            "price_position": {
                ...
                "ma20": MA20 숫자,
                "bias_ma5": 이격률 퍼센트 숫자,
                "bias_status": "안전/주의/위험",
                "support_level": 지지선 가격,
                "resistance_level": 저항선 가격
            },
            "volume_analysis": {
                "volume_ratio": 거래량 비율 숫자,
                "volume_status": "거래량증가/거래량감소/보합",
                "turnover_rate": 회전율 퍼센트,
                "volume_meaning": "거래량 의미 해석 (예: 거래량 감소 조정은 매도 압력 완화)"
            },
            "chip_structure": {
                "profit_ratio": 수익 비율,
                "avg_cost": 평균 원가,
                "concentration": 수급 집중도,
                "chip_health": "건강/보통/주의"
            }
        },

        "intelligence": {
            "latest_news": "【최신 정보】최근 중요 뉴스 요약",
            "risk_alerts": ["위험 1: 구체적 설명", "위험 2: 구체적 설명"],
            "positive_catalysts": ["호재 1: 구체적 설명", "호재 2: 구체적 설명"],
            "earnings_outlook": "실적 전망 분석 (연간보고 예고, 실적 공시 등 기반)",
            "sentiment_summary": "여론 심리 한 줄 요약"
        },

        "battle_plan": {
            "sniper_points": {
                "ideal_buy": "최적 매수 포인트: XX원 (MA5 부근)",
                "secondary_buy": "차선 매수 포인트: XX원 (MA10 부근)",
                "stop_loss": "손절선: XX원 (MA20 하향 이탈 또는 X%)",
                "take_profit": "목표가: XX원 (전고점/정수 저항선)"
            },
            "position_strategy": {
                "suggested_position": "권장 포지션: X할",
                "entry_plan": "분할 매수 전략 설명",
                "risk_control": "리스크 관리 전략 설명"
            },
            "action_checklist": [
                "✅/⚠️/❌ 체크1: 정배열",
                "✅/⚠️/❌ 체크2: 이격률 안전 (강세 추세는 조건 완화)",
                "✅/⚠️/❌ 체크3: 거래량 수반",
                "✅/⚠️/❌ 체크4: 중대 악재 없음",
                "✅/⚠️/❌ 체크5: 수급 건강",
                "✅/⚠️/❌ 체크6: PER 밸류에이션 합리적"
...
| MA10 | {today.get('ma10', 'N/A')} | 중단기 추세선 |
| MA20 | {today.get('ma20', 'N/A')} | 중기 추세선 |
| 이동평균선 형태 | {context.get('ma_status', '알 수 없음')} | 정배열/역배열/수렴 |
"""
...
| 평균 원가 | {chip.get('avg_cost', 'N/A')} | 현재가가 5-15% 높아야 |
| 90% 수급 집중도 | {chip.get('concentration_90', 0):.2%} | <15%이면 집중 |
...
"""
        ...
        if 'trend_analysis' in context:
            ...
            bias_warning = "🚨 5% 초과, 고점 추격 절대 금지!" if trend.get('bias_ma5', 0) > 5 else "✅ 안전 범위"
            prompt += f"""
### 추세 분석 예측 (매매 철학 기반)
| 지표 | 수치 | 판정 |
|------|------|------|
| 추세 상태 | {trend.get('trend_status', '알 수 없음')} | |
| 이동평균선 배열 | {trend.get('ma_alignment', '알 수 없음')} | MA5>MA10>MA20이면 정배열 |
...
"""
        if news_context:
            prompt += f"""
다음은 **{stock_name}({code})** 최근 7일 뉴스 검색 결과입니다. 다음 항목을 중점적으로 추출하세요:
1. 🚨 **리스크 경보**: 지분 매도, 처벌, 악재
2. 🎯 **호재 촉매**: 실적, 계약, 정책
3. 📊 **실적 전망**: 연간보고 예고, 실적 공시
...
"""
            ...
            prompt += """
해당 종목의 최근 관련 뉴스를 찾지 못했습니다. 주로 기술적 데이터를 기반으로 분석해 주세요.
"""

        if context.get('data_missing'):
            prompt += """
⚠️ **데이터 누락 경고**
인터페이스 제한으로 인해 현재 완전한 실시간 시세 및 기술적 지표 데이터를 가져올 수 없습니다.
위 표의 **N/A 데이터는 무시**하고, 주로 **【📰 뉴스/여론 정보】** 의 뉴스를 기반으로 펀더멘털 및 심리 분석을 진행해 주세요.
기술적 질문(이동평균선, 이격률 등)에 답변할 때는 "데이터 없음, 판단 불가"라고 직접 명시하고, **데이터를 임의로 생성하는 것은 절대 금지**합니다.
"""
...
**{stock_name}({code})** 에 대한 【의사결정 대시보드】를 생성해 주세요. JSON 형식을 엄격히 따르고, 보고서 및 JSON 내 모든 설명, 결론, 조언은 **한국어**로 작성합니다.
"""
        if context.get('is_index_etf'):
            prompt += """
> ⚠️ **지수/ETF 분석 제약**: 해당 종목은 지수 추종 ETF 또는 시장 지수입니다.
> - 리스크 분석은 다음에만 집중: **지수 추세, 추적 오차, 시장 유동성**
> - 펀드사의 소송, 평판, 임원 변동을 리스크 경보에 포함하지 마세요
> - 실적 전망은 **지수 구성 종목 전체 성과** 기반이며, 펀드사 재무제표가 아닙니다
> - `risk_alerts`에 펀드 운용사 관련 경영 리스크를 포함하지 마세요

"""
        prompt += f"""
### ⚠️ 중요: 올바른 종목명 형식 출력
종목명 형식은 "종목명(종목코드)" 형태로 출력하세요.
위에 표시된 종목명이 올바르지 않다면, 분석 시작 시 **해당 종목의 올바른 명칭을 명확히 출력**해 주세요.

### 중점 확인 사항 (반드시 명확히 답변):
1. ❓ MA5>MA10>MA20 정배열 충족 여부?
2. ❓ 현재 이격률이 안전 범위(<5%)인가? — 5% 초과 시 반드시 "고점 추격 금지" 표시
3. ❓ 거래량 수반 여부(거래량 감소 조정/거래량 증가 돌파)?
4. ❓ 수급 구조가 건강한가?
5. ❓ 뉴스에 중대 악재가 있는가? (지분 매도, 처벌, 실적 급변 등)

### 의사결정 대시보드 요구사항:
- **종목명**: 올바른 명칭 출력 필수
- **핵심 결론**: 한 줄로 매수/매도/관망 명확히
- **포지션별 맞춤 조언**: 미보유자 vs 보유자 구분
- **구체적 가격 포인트**: 매수가, 손절가, 목표가 (소수점까지)
- **체크리스트**: 각 항목에 ✅/⚠️/❌ 표시

완전한 JSON 형식의 의사결정 대시보드를 출력해 주세요."""
