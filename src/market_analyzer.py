...
"""
        ...
        return f"""당신은 A주/홍콩주/미국주 전문 시장 분석가입니다. 아래 데이터를 바탕으로 간결한 장 마감 리뷰 보고서를 생성해 주세요.

【중요】출력 요구사항:
- **출력 언어**: 보고서 전문(제목, 단락, 요약, 전망, 리스크 경고 등)은 반드시 **한국어**로 작성합니다.
- 순수 Markdown 텍스트 형식으로 출력
- JSON 형식 출력 금지
- 코드 블록 출력 금지
- 이모지는 제목에만 소량 사용 (제목당 최대 1개)
...
# 출력 형식 템플릿 (반드시 이 형식을 따르세요)

## {overview.date} 장 마감 리뷰

### 1. 시장 총괄
(오늘 전체 시장 성과 2-3문장 요약, 지수 등락 및 거래량 변화 포함)
...
### 4. 테마 분석
(상승/하락 섹터의 논리와 주도 요인 분석)

### 5. 향후 전망
(현재 추세와 뉴스를 바탕으로 내일 시장 예측 제시)

### 6. 리스크 경고
(주목해야 할 리스크 포인트)

### 7. 전략 계획
(공격/균형/방어 결론, 해당 포지션 조언, 무효화 조건 1개 제시; 마지막에 "본 조언은 참고용이며 투자 조언이 아닙니다." 추가)

---

보고서 내용만 직접 출력하고 다른 설명은 넣지 마세요.
"""
